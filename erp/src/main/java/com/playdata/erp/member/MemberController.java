...
public class MemberController {
	...
	// Session object를 만들기 위해 parameter에 HeepServletRequest를 추가
	@PostMapping("/login")
	public String login(MemberDTO loginUser, HttpServletRequest request) {
		MemberDTO user = serv.login(loginUser);
		// login 여부에 따라 view가 달라질 것이므로
		String view;
		if (user != null) {// login 성공
			// make session
			// -> 기존에 사용하던 session이 있으면 그 session object를 return, 없으면 새로 만들어서 return.
	...
	// @sessionAttributes를 이용해서 login 처리를 한 경우 SessionStatus object의 method를 이용해서
	// 처리한다.
	// session에 저장된 object를 제거하기 위한 object
